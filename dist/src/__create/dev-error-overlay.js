(() => {
    ...
    `;
        ...
        /* button wiring --------------------------------------------------- */
        panel.querySelector('#copy').onclick = () => navigator.clipboard.writeText(`${msg}\n${stack || ''}`);
    ...
    function sendFixMessage() {
        window.parent?.postMessage({ type: 'sandbox:web:fix', error: { message: lastError?.message } }, '*');
        clear();
    }
    /* vite HMR channel ------------------------------------------------- */
    if (import.meta.hot) {
        ...
        ['vite:beforeUpdate', 'vite:afterUpdate', 'vite:beforeFullReload'].forEach((evt) => import.meta.hot.on(evt, clear));
    }
})();
export {};
