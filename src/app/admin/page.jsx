...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      setQuoteRequests(quotesData.quotes || []);
                  ...
                  {quoteRequests.slice(0, 5).map((quote) => (
                    <li key={quote.id} className="py-4">
                      ...
                      <p className="text-sm text-gray-500">
                        Phone: {quote.mobile_no || "N/A"}
                      </p>
                      <p className="text-sm text-gray-500">
                        {quote.service || "No service"} - {quote.budget || "No budget"}
                      </p>
                  ...
                  {quoteRequests.map((quote) => (
                    <tr key={quote.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {quote.service ? (
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getServiceBadgeColor(
                              quote.service
                            )}`}
                          >
                            {quote.service}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {quote.budget ? (
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getBudgetBadgeColor(
                              quote.budget
                            )}`}
                          >
                            {quote.budget}
