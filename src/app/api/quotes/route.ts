...
quotes.post('/', async (c) => {
  try {
    ...
    name = name?.trim();
    email = email?.trim().toLowerCase();
    mobile_no = mobile_no?.replace(/\s+/g, '').replace(/^(\+91)/, '');
    company = company?.trim();
    service = service?.trim();
    budget = budget?.trim();
    message = message?.trim();
    ...
    if (!rows?.length) return c.json({ error: 'Insert failed' }, 500);
